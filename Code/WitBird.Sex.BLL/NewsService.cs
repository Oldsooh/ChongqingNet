...
namespace WitBird.Sex.BLL
{
    public class NewsService
    {
        ...
        public List<News> GetRecommendNews()
        {
            ...
            {
                conn.Open();
                result = DBRepository.SelectMore<News>("IsRecommend = 1 and IsActive = 1", "Id", false, conn, 10);
            }
            catch (Exception e)
            {
        ...
        public List<News> GetSlideNews()
        {
            ...
            {
                conn.Open();
                result = DBRepository.SelectMore<News>("IsActive = 1 and PictureGroup !=''", "Id", false, conn, 10);
            }
            catch (Exception e)
            {
        ...
        public List<News> GetHotNews()
        {
            ...
            try
            {
                ...
                result = DBRepository.SelectMore<News>("IsActive = 1 and UpdateTime >'" + dt + "'", "ViewCount", false, conn, 10);
            }
            catch (Exception e)
            {
                LogService.Log("热门列表", e.ToString());
        ...
        public List<News> GetRandomNews()
        {
            ...
            {
                conn.Open();
                string sql = "select top 10  * from News where IsActive=1 order by newid()";
            ...
            try
            {
                ...
                if (news != null && news.IsActive && !string.IsNullOrEmpty(news.Keywords))
                {
                    if (!string.IsNullOrEmpty(news.Keywords))
                    {
                        ...
                        result = DBRepository.SelectMore<News>(where, "Id", false, conn, 10);
                    }
                }
            }
            catch (Exception e)
