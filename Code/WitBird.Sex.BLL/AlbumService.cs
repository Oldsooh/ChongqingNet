...
namespace WitBird.Sex.BLL
{
    public class AlbumService
    {
        ...
        public List<Album> GetAlbumList(string categoryId, string keywords, int pageCount, int pageIndex, out int count, int isShow = 1)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(where))
                {
                    where += " and IsDelete = 0  and IsShow = " + isShow;
                }
                else
                {
                    where = "IsDelete = 0 and IsShow = " + isShow;
                }
                result = DBRepository.SelectMore<Album>(where, "InsertTime", false, pageCount, pageIndex, out tempCount, conn);
                count = tempCount;
            }
            catch (Exception e)
            {
        ...
        public Album GetAlbumById(int id)
        {
            ...
            {
                conn.Open();
                album = DBRepository.SelectOne<Album>("Id=" + id.ToString() + " and IsDelete=0", conn);
                if (album != null)
                {
        ...
        public List<Album> GetAlbumByCategoryId(string categoryId, int pageCount, int pageIndex, out int count)
        {
            ...
            try
            {
                conn.Open();
                int tempCount = 0;
                albums = DBRepository.SelectMore<Album>("CategoryId='" + categoryId + "' and IsDelete=0 and IsShow=1", "Id", false, pageCount, pageIndex, out tempCount, conn);
                count = tempCount;
            }
            catch (Exception e)
            {
        ...
        public List<Album> GetAlbumByNew()
        {
            ...
            {
                conn.Open();
                albums = DBRepository.SelectMore<Album>("IsDelete=0 and IsShow=1", "InsertTime", false, conn, 10);
            }
            catch (Exception e)
            {
        ...
        public AlbumDetail GetAlbumDetail(int id)
        {
            ...
            try
            {
                ...
                if (album != null)
                {
                    ...
                    sqlBuilder.Append(";");
                    //Last Album
                    sqlBuilder.Append("select top 1 * from Album where IsDelete=0 and IsShow=1 and Id < ");
                    sqlBuilder.Append(album.Id);
                    ...
                    {
                        sqlBuilder.Append("select top 12 * from Album where (");
                        char[] FilterChar = {' ', ',', '，' };
                        string[] keywords = album.Keywords.Split(FilterChar);
                    ...
                    sqlBuilder.Append("select top 10 * from Album where IsTop=1 and IsDelete=0 and IsShow = 1 order by newid();");
